...
//    private static final long TIME_INTERVAL = 7 * 24 * 60 * 60 * 1000L;
    ...
    public static String getForegroundActivityName() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            ActivityManager manager = (ActivityManager) MainApplication.mAppContext.getApplicationContext().getSystemService(Context.ACTIVITY_SERVICE);
            currentClassName = manager.getRunningTasks(1).get(0).topActivity.getPackageName();
        } else {
            UsageStats initStat = getForegroundUsageStats(MainApplication.mAppContext, START_TIME, END_TIME);
            if (initStat != null) {
